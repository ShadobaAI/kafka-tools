...
Примеры Kafka Adapter разделены на два самостоятельных файла:

- [`xdto/kfk-test.yaml`](xdto/kfk-test.yaml) — общий тестовый и демонстрационный пример обмена: справочник и регистры.
- [`xdto/kfk-unit.yaml`](xdto/kfk-unit.yaml) — пример преобразования типов `TestConversion` и вспомогательных схем.

В тестах допускается использовать оба YAML. Каждый файл передаётся генератору отдельно.
Для самостоятельной генерации второй файл содержит копию `TestCatalog`, на которую ссылаются поля `catalog` и `catalogs`; определение совпадает с общим примером.
При изменении `TestCatalog` поддерживайте обе копии согласованными. Внешние `$ref` между файлами генератор не поддерживает.
