...
3. Apply the semantic object-fill gate, then set material and related values through `Установить(...)` or `УстановитьРеквизиты(...)`. Build each table-part row for its scenario role through `ТабличнаяЧасть(...)` and `ДобавитьСтроку(...)`; call `Объект()` before returning from a table-part row to object attributes.
