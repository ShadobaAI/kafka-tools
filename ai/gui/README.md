...
Node.js проверяется до запуска операции: при отсутствии/неподдерживаемой версии GUI предлагает Node.js LTS (https://nodejs.org/), перезапуск GUI и node --version. Codex CLI проверяется для действий, которые читают effective configuration; ошибка содержит npm install -g @openai/codex. Ошибки PowerShell передаются в журнал GUI.

При полной установке в открывшейся консоли проверяется доверие Codex к целевым проектам. Если его нет, подтвердите `y/yes`: установщик запишет доверие через API Codex в активный пользовательский config.toml. Уже доверенные проекты проходят без вопросов и перезаписи. Проверка выполняется до runtime и перестроения индексов. Скрипты берутся из каталога toolkit при каждом запуске EXE.
