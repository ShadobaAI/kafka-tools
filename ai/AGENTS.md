...
- Prefer the shared `code-index` MCP over EDT for read-only project information, search, and analysis whenever slight index staleness is acceptable, for both narrow and broad questions. Use it for indexed discovery, source and metadata structure, call/data graphs, references, and impact analysis. Bind the exact repository alias, check `health` when freshness is not established, and treat the index as eventually consistent evidence.
