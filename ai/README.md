...
Doctor читает настройки MCP через Codex app-server config/read с явным cwd проекта. В отличие от списка MCP, ответ config/read также позволяет определить отключённый проектный слой. Пустой codex mcp list при существующем .codex/config.toml может означать, что проект не доверенный в текущем CODEX_HOME. Отключённый проектный слой диагностируется отдельно; доверие автоматически не меняется. Требуется CLI с поддержкой config/read(cwd). Конфигурация читается один раз на каталог за запуск doctor.

Если doctor сообщает об отключённом проектном слое, откройте `codex` из корня устанавливаемого проекта и подтвердите доверие к этому проекту. Затем повторите doctor. Если доверие уже подтверждено, проверьте точный путь проекта и используемый `CODEX_HOME`. Установщик не меняет доверие автоматически. Для статической проверки чтения конфигурации: `node tests/test-codex-config.mjs` из каталога toolkit; дополнительный изолированный тест установленного CLI: та же команда с `--live-cli-fixture` (требует Codex и Git; MCP не запускаются).
