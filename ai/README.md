...
Контекст загружается по этапам: routing → нужный skill → для изменения только
`1c-code-change/references/requirements.md` → подходящие подразделы v8std.
Перед записью проверяется фактический состав изменения и догружается только
разница требований; после записи используется тот же набор. Полные правила не
