...
Для существующей базы совместимое обновление OpenViking сохраняет Docker volume
и индекс. Перед обновлением и после него проверяется согласованность файлов и
векторов; неизменённые документы не индексируются повторно. Код допускает
переход 0.4.20 → 0.4.21 после этих проверок. Если версия или состояние не проходят проверку,
установщик останавливается без автоматической полной перестройки.
...
Doctor читает настройки MCP через Codex app-server config/read с явным cwd проекта. В отличие от списка MCP, ответ config/read также позволяет определить отключённый проектный слой. Пустой codex mcp list при существующем .codex/config.toml может означать, что проект не доверенный в текущем CODEX_HOME. Отключённый проектный слой диагностируется отдельно. Требуется CLI с поддержкой config/read(cwd). Конфигурация читается один раз на каталог за запуск doctor.

Если doctor сообщает об отключённом проектном слое, откройте `codex` из корня устанавливаемого проекта и подтвердите доверие к этому проекту. Затем повторите doctor. Если доверие уже подтверждено, проверьте точный путь проекта и используемый `CODEX_HOME`. При полной установке доверие проверяется до runtime и индексов: для недоверенных проектов консоль установщика (в том числе при запуске из EXE) показывает пути и запрашивает `y/yes`. После согласия Codex API сохраняет доверие в пользовательском config.toml активного CODEX_HOME. Уже доверенные проекты не требуют подтверждения или перезаписи. Эквивалентный путь нормализуется без дублирования секции; конфликтующие дубликаты останавливают установку. При отказе установка останавливается. ConfigurationOnly и SkipDaemonStart не меняют доверие. Решение о доверии сохраняется при последующем rollback toolkit. Для статической проверки чтения конфигурации: `node tests/test-codex-config.mjs` из каталога toolkit; дополнительный изолированный тест установленного CLI: та же команда с `--live-cli-fixture` (требует Codex и Git; MCP не запускаются).

Проверка идемпотентности доверия: `node tests/test-project-trust.mjs`; изолированная проверка API установленного CLI: добавить `--live-cli-fixture`. Реальный пользовательский профиль эти тесты не меняют.
