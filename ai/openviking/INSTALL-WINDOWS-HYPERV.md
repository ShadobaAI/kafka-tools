...
При сообщении `mode: external Ollama` установщик предлагает загрузить образ
OpenViking и запустить сервис Kafka. Если сервис уже работает, запрос сообщает,
что существующий OpenViking будет обновлён до последнего стабильного выпуска,
если он совместим с сохранённым индексом; Docker volume и индекс останутся на
месте, полная повторная индексация не запускается. Контейнер может быть
пересоздан. При неподтверждённой совместимости установщик остановится без
удаления базы. Для продолжения подтвердите `y`.
Ответ `n` останавливает всю установку, а не только обновление OpenViking.
Модели на хосте повторно скачиваться не будут.
